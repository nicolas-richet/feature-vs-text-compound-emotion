            ...
            for batch_size in constants.PARAM_GRID['batch_size']:
                for lr in constants.PARAM_GRID['lr']:
                    ...
                    for split_dict in split_dict_list:
                        model_name = f'{base_model_name}_{datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}'
                        ...
                        sampler = None
                        train_dataloader = DataLoader(split_dict['train'], sampler = sampler, batch_size=batch_size)
                        ...
                        if constants.DATASET == 'C-EXPR-DB':
                            valid_f1_w_other = [x['Valid. F1 w. Other'] for x in training_stats]
                            valid_f1_wo_other = [x['Valid. F1 wo. Other'] for x in training_stats]
                            fig = plt.figure(figsize = (12,8))
                            f1_ax = fig.add_subplot(2,1,1)
                            f1_ax.plot(range(len(valid_f1_w_other)), valid_f1_w_other, label ='Validation F1 w. Other')
                        ...
                        acc_ax.legend()
                        plt.savefig(f'{constants.UNIMODAL_TEXT_OUTPUT_PATH}/LossEvolution{datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}')
                        plt.close()                    
                        training_stats_df = pd.DataFrame.from_records(training_stats)
                        training_stats_df.to_pickle(f'{constants.UNIMODAL_TEXT_OUTPUT_PATH}/stats_{datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}.pkl')
                        with open(f'{constants.UNIMODAL_TEXT_OUTPUT_PATH}/results.txt', 'a') as result_file:
                            if constants.DATASET == 'C-EXPR-DB':
                                log = f'\nbest val f1 wo. Other {np.max(valid_f1_wo_other)} split:{split_name} window_size:{constants.WINDOW_SIZE} BZ:{batch_size} LR:{lr} best val f1 w. Other {np.max(valid_f1_w_other)} TRANSCRIPTION_CONTEXT: {constants.TRANSCRIPTION_CONTEXT} MODEL:{model_name} SEED:{seed} wd:{constants.PARAM_GRID["weight_decay"]} {datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}  lora_r:{constants.LORA_R} lora_alpha:{constants.LORA_ALPHA} lora_dropout:{constants.LORA_DROPOUT} max grad norm:{constants.MAX_GRAD_NORM} samples_per_epochs:{constants.SAMPLES_PER_EPOCHS}'
                            elif constants.DATASET == 'MELD':
                                log = f'\nbest val f1 {np.max(valid_f1)} 100%/single_frame {constants.USE_SINGLE_FRAME_PER_VIDEO} split:{split_name} window_size:{constants.WINDOW_SIZE} BZ:{batch_size} LR:{lr}  TRANSCRIPTION_CONTEXT: {constants.TRANSCRIPTION_CONTEXT} MODEL:{model_name} SEED:{seed} wd:{constants.PARAM_GRID["weight_decay"]} {datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S")}  lora_r:{constants.LORA_R} lora_alpha:{constants.LORA_ALPHA} lora_dropout:{constants.LORA_DROPOUT} max grad norm:{constants.MAX_GRAD_NORM} samples_per_epochs:{constants.SAMPLES_PER_EPOCHS}'
